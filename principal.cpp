...
int main()
{
    ...
    while (seguir)
    {
        ...
        { // Crea una fecha y la almacena en la variable fecha
            main_crearFecha(fecha);
        }
        else if (cmd_es("imprimirFecha", cmd))
        { // Imprime la fecha almacenada en la variable fecha
            main_imprimirFecha(fecha);
        }
        else if (cmd_es("liberarFecha", cmd))
        { // Libera la memoria de la variable fecha
            main_liberarFecha(fecha);
        }
        else if (cmd_es("aumentarDias", cmd))
        { // Aumenta la fecha almacenada en la variable fecha en la cantidad de días indicada
            main_aumentarDias(fecha);
        }
        else if (cmd_es("compararFechas", cmd))
        { // Compara dos fechas leidas de la entrada
            main_compararFechas();
        ...
        { // Crea una persona y la almacena en la variable persona
            main_crearPersona(persona, fecha);
            fecha = NULL;
        }
        else if (cmd_es("imprimirCiPersona", cmd))
        { // Imprime la ci de la persona almacenada en la variable persona
            main_imprimirCiPersona(persona);
        }
        else if (cmd_es("imprimirFechaPersona", cmd))
        { // Imprime la fecha de nacimiento de la persona almacenada en la variable persona
            main_imprimirFechaPersona(persona);
        }
        else if (cmd_es("imprimirNombreYApellidoPersona", cmd))
        { // Imprime nombre y apellido de la persona almacenada en la variable persona
            main_imprimirNombreYApellidoPersona(persona);
        }
        else if (cmd_es("imprimirPersona", cmd))
        { // Imprime toda la infromación de la persona almacenada en la variable persona
            main_imprimirPersona(persona);
        }
        else if (cmd_es("liberarPersona", cmd))
        { // Libera la memoria de la persona almacenada en la variable persona
            main_liberarPersona(persona);
        }
        else if (cmd_es("esMasJovenPersona", cmd))
        { // Evalúa si la persona almacenada en persona es más joven que la ingresada
        ...
        else if (cmd_es("crearGrupo", cmd))
        { // Crea un grupo y la almacena en la variable grupo
            main_crearGrupo(grupo);
        }
        else if (cmd_es("agregarAGrupo", cmd))
        { // Agrega la persona almacenada en la variable persona al grupo almacenado en la variable grupo
            main_agregarAGrupo(grupo, persona);
            persona = NULL;
        }
        else if (cmd_es("imprimirGrupo", cmd))
        { // Imprime toda la información del grupo almacenado en la variable grupo
            main_imprimirGrupo(grupo);
        }
        else if (cmd_es("liberarGrupo", cmd))
        { // Libera la memoria del grupo almacenado en la variable grupo
            main_liberarGrupo(grupo);
        }
        else if (cmd_es("estaEnGrupo", cmd))
        { // Indica si el grupo almacenado en la variable grupo tiene a la persona con ci ingresada
            main_estaEnGrupo(grupo);
        }
        else if (cmd_es("imprimirPersonasFecha", cmd))
        { // Imprime las personas pertenecientes al grupo que tengan la fecha de nacimiento ingresada
            main_imprimirPersonasFecha(grupo);
        }
        else if (cmd_es("hayPersonasFecha", cmd))
        { // Indica si hay personas en el  grupo en una fecha indicada
            main_hayPersonasFecha(grupo);
        }
        else if (cmd_es("removerDeGrupo", cmd))
        { // Remueve la persona con la cédula ingresada de la variable grupo
